...
data||$this.data("bs.affix",data=new Affix(this,options)),"string"==typeof option&&data[option]()})},$.fn.affix.Constructor=Affix,$.fn.affix.noConflict=function(){return $.fn.affix=old,this},$(window).on("load",function(){$('[data-spy="affix"]').each(function(){var $spy=$(this),data=$spy.data();data.offset=data.offset||{},data.offsetBottom&&(data.offset.bottom=data.offsetBottom),data.offsetTop&&(data.offset.top=data.offsetTop),$spy.affix(data)})})}(jQuery),angular.module("meetingsApp",["ngRoute","LocalStorageModule"]).filter("escape",function(){return window.escape}).config(["$routeProvider","$locationProvider",function($routeProvider,$locationProvider){$routeProvider.when("/",{templateUrl:"/partials/meetings.html",controller:"meetingsCtrl"}).when("/help",{templateUrl:"/partials/help.html",controller:"helpCtrl"}).when("/meetings/:slug",{templateUrl:"/partials/meeting-detail.html",controller:"meetingDetailCtrl"}).otherwise({redirectTo:"/"}),$locationProvider.html5Mode(!0)}]).run(["$rootScope","$http","localStorageService",function($rootScope,$http,localStorageService){function foundLocation(position){$rootScope.userLocation=position.coords}function noLocation(){$rootScope.userLocation=!1}$rootScope.days=[{id:0,value:"Sunday"},{id:1,value:"Monday"},{id:2,value:"Tuesday"},{id:3,value:"Wednesday"},{id:4,value:"Thursday"},{id:5,value:"Friday"},{id:6,value:"Saturday"}],$rootScope.regions=localStorageService.get("regions")||new Array,$rootScope.meetings=localStorageService.get("meetings")||new Array,$rootScope.favorites=localStorageService.get("favorites")||new Array,$rootScope.userLocation=null,window.navigator.standalone&&navigator.geolocation.getCurrentPosition(foundLocation,noLocation,{timeout:1e4}),$http.get("http://aasanjose.org/wp-admin/admin-ajax.php?action=meetings").success(function(data){function compare(a,b){return a.value<b.value?-1:a.value>b.value?1:0}$rootScope.meetings=data;for(var regions=new Array,regionKeys=new Array,i=0;i<$rootScope.meetings.length;i++)$rootScope.meetings[i].types_formatted=$rootScope.format_types($rootScope.meetings[i].types),$rootScope.meetings[i].favorite=$rootScope.favorites?-1!=$rootScope.favorites.indexOf($rootScope.meetings[i].id):!1,-1==regionKeys.indexOf($rootScope.meetings[i].region)&&(regionKeys[regionKeys.length]=$rootScope.meetings[i].region,regions[regions.length]={id:$rootScope.meetings[i].region_id,value:$rootScope.meetings[i].region});regions.sort(compare),$rootScope.regions=regions,localStorageService.add("meetings",$rootScope.meetings),localStorageService.add("regions",regions)}),$rootScope.format_types=function(types){for(var i=0;i<types.length;i++){if("M"==types[i])return"Men";if("W"==types[i])return"Women"}return""}}]).controller("meetingsCtrl",["$scope",function($scope){$scope.now=new Date,$scope.selected_day=$scope.now.getDay(),$scope.selected_region="",$scope.$watch("filteredMeetings",function(){$scope.scroll_to_now()},!0),$scope.get_next_meeting=function(){if("undefined"==typeof $scope.filteredMeetings)return!1;var timestring=($scope.now.getHours()<10?"0":"")+$scope.now.getHours()+":";timestring+=($scope.now.getMinutes()<10?"0":"")+$scope.now.getMinutes();for(var i=0;i<$scope.filteredMeetings.length;i++)if($scope.filteredMeetings[i].time>=timestring)return $scope.filteredMeetings[i];return!1},$scope.scroll_to_now=function(){var target=(meeting=$scope.get_next_meeting())?$("#meeting-"+meeting.id).offset().top-64:0,max=$("div.height").height();max<=$(window).height()-64&&(target=0),target>max&&(target=max),$("html,body").animate({scrollTop:target},0)},$scope.reset_vars=function(){$scope.selected_day=$scope.now.getDay(),$scope.selected_region="",$scope.scroll_to_now(),$("#collapse").hasClass("in")&&$("#collapse").collapse("hide")},$scope.format_day=function(){return $scope.days[$scope.selected_day].value},$scope.format_region=function(){if(""==$scope.selected_region)return"";for(var i=0;i<$scope.regions.length;i++)if($scope.regions[i].id==$scope.selected_region)return $scope.regions[i].value},$scope.format_time=function(time){var time_parts=time.split(":"),hours=time_parts[0],minutes=time_parts[1];return 12==hours&&0==minutes?"Noon":23==hours&&59==minutes?"Mid":12>hours?hours-0+":"+minutes+"a":12==hours?"12:"+minutes+"p":hours-12+":"+minutes+"p"},$scope.set_day=function(day){$scope.selected_day=day.id,$("#collapse").collapse("hide")},$scope.set_region=function(region){$scope.selected_region="undefined"==typeof region?"":region.id,$("#collapse").collapse("hide")}}]).controller("meetingDetailCtrl",["$scope","$rootScope","$routeParams","$window","$location","localStorageService",function($scope,$rootScope,$routeParams,$window,$location,localStorageService){function deg2rad(deg){return deg*(Math.PI/180)}$scope.$on("$viewContentLoaded",function(){$window.ga("send","pageview",{page:$location.path()})}),$("html,body").animate({scrollTop:0},0),$scope.getMeeting=function(slug){for(var i=0;i<$scope.meetings.length;i++)if($scope.meetings[i].slug==slug)return $scope.meetings[i]},$scope.meeting=$scope.getMeeting($routeParams.slug),$scope.getDistanceInMi=function(){if($scope.userLocation===!1)return!1;if(null===$scope.userLocation)return null;var lat1=$scope.userLocation.latitude,lon1=$scope.userLocation.longitude,lat2=$scope.meeting.latitude,lon2=$scope.meeting.longitude,R=6371,dLat=deg2rad(lat2-lat1),dLon=deg2rad(lon2-lon1),a=Math.sin(dLat/2)*Math.sin(dLat/2)+Math.cos(deg2rad(lat1))*Math.cos(deg2rad(lat2))*Math.sin(dLon/2)*Math.sin(dLon/2),c=2*Math.atan2(Math.sqrt(a),Math.sqrt(1-a)),d=R*c;return Math.round(.621371*d*100)/100},$scope.meeting.distance=$scope.getDistanceInMi(),$scope.types={H:"Chips",C:"Closed",G:"Gay",L:"Lesbian",M:"Men Only",O:"Open",S:"Spanish",X:"Wheelchair Accessible",W:"Women Only",Y:"Young People"},$scope.set_favorite=function(){if($scope.meeting.favorite===!0){var i=$scope.favorites.indexOf($scope.meeting.id);-1!=i&&$scope.favorites.splice(i,1),$scope.meeting.favorite=!1}else{$scope.meeting.favorite=!0;var i=$scope.favorites.indexOf($scope.meeting.id);-1==i&&($scope.favorites[$scope.favorites.length]=$scope.meeting.id)}for(var i=0;i<$scope.meetings.length;i++)$scope.meetings[i].id==$scope.meeting.id&&($scope.meetings[i].favorite=$scope.meeting.favorite);localStorageService.add("meetings",$scope.meetings),localStorageService.add("favorites",$scope.favorites)}}]).controller("helpCtrl",["$window","$location",function($window,$location){$scope.$on("$viewContentLoaded",function(){$window.ga("send","pageview",{page:$location.path()})}),$("html,body").animate({scrollTop:0},0)}]),addToHomescreen(),function(b,o,i,l,e,r){b.GoogleAnalyticsObject=l,b[l]||(b[l]=function(){(b[l].q=b[l].q||[]).push(arguments)}),b[l].l=+new Date,e=o.createElement(i),r=o.getElementsByTagName(i)[0],e.src="//www.google-analytics.com/analytics.js",r.parentNode.insertBefore(e,r)}(window,document,"script","ga"),ga("create","UA-80350-22"),ga("send","pageview");